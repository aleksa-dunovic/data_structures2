...
void heapify_up(Heap *h, int i) {
	...
	if (compare_data(&h->array[Parent(i)], &h->array[Parent(Parent(i))]) == 1)
...
int get_leaves(Heap *h, int *leaves_array) {
	...
	for (int i = 0; i < h->size; i++) {
		...
		if (Left(i) >= h->size && Right(i) >= h->size) {
			//check if index of children is included in the size of the array
			leaves_array[count] = i;
			count++;
		}
	}
	return count;
...
Data* find_max_heap(Heap *h) {
	...
	return copy_data(&h->array[0]); //root is max
}
Data* find_min_heap(Heap *h) {
	...
	//loop through array and find min
	Data *min = &h->array[0];
	for (int i = 0; i < h->size; i++) {
...
void heap_sort2(Data *array, const int size) {
	...
	//remove items from heap and place them into array starting at the back of the array
	for (i = size - 1; i >= 0; i--) {
		array[i] = *peek_heap(h);
		remove_heap(h);
	}
